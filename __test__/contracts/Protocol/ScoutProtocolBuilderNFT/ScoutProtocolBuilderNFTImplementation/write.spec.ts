  ...
  describe('updateERC20Contract()', function () {
    describe('effects', function () {
      it('updates the ERC20 contract address', async function () {
        ...
        await scoutProtocolBuilderNFT.builderNftContract.write.updateERC20Contract([newERC20Address], {
          account: erc1155AdminAccount.account
        });

        const erc20Contract = await scoutProtocolBuilderNFT.builderNftContract.read.ERC20Token();
    ...
    describe('events', function () {
      it('emits ERC20ContractUpdated event', async function () {
        const previousERC20Contract = await scoutProtocolBuilderNFT.builderNftContract.read.ERC20Token();
        const newERC20Address = randomEthereumAddress();

        const txResponse = await scoutProtocolBuilderNFT.builderNftContract.write.updateERC20Contract(
          [newERC20Address],
          {
            account: erc1155AdminAccount.account
          }
    ...
    describe('permissions', function () {
      it('only admin can update the ERC20 contract address', async function () {
        ...
        await expect(
          scoutProtocolBuilderNFT.builderNftContract.write.updateERC20Contract([newERC20Address], {
            account: userAccount.account
          })
        ).rejects.toThrow('Caller is not the admin');
      });
    ...
    describe('validations', function () {
      it('reverts when setting to the zero address', async function () {
        await expect(
          scoutProtocolBuilderNFT.builderNftContract.write.updateERC20Contract(
